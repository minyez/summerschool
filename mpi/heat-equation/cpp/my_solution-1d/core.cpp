...
//! Send/Recv version
static void exchange_send_recv(Field& field, const ParallelData parallel)
{
    double* sbuf;
    double* rbuf;
    MPI_Status status;
    ...
    if (parallel.rank % 2)
    {
        ...
        MPI_Recv(rbuf, field.ny + 2, MPI_DOUBLE, parallel.ndown, 0, MPI_COMM_WORLD, &status);
    }
    else
    {
        MPI_Recv(rbuf, field.ny + 2, MPI_DOUBLE, parallel.ndown, 0, MPI_COMM_WORLD, &status);
    ...
    if (parallel.rank % 2)
    {
        ...
        MPI_Recv(rbuf, field.ny + 2, MPI_DOUBLE, parallel.nup, 0, MPI_COMM_WORLD, &status);
    }
    else
    {
        MPI_Recv(rbuf, field.ny + 2, MPI_DOUBLE, parallel.nup, 0, MPI_COMM_WORLD, &status);
...
static void exchange_sendrecv(Field& field, const ParallelData parallel)
{
    ...
    sbuf = &field.temperature(1, 0);
    rbuf = &field.temperature(field.nx + 1, 0);
    MPI_Sendrecv(sbuf, field.ny + 2, MPI_DOUBLE, parallel.nup, 0, rbuf, field.ny + 2, MPI_DOUBLE, parallel.ndown, 0, MPI_COMM_WORLD, &status);

    // Send to down, receive from up
    sbuf = &field.temperature(field.nx, 0);
    rbuf = &field.temperature(0, 0);
    MPI_Sendrecv(sbuf, field.ny + 2, MPI_DOUBLE, parallel.ndown, 0, rbuf, field.ny + 2, MPI_DOUBLE, parallel.nup, 0, MPI_COMM_WORLD, &status);
}
